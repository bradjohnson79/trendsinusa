...
import type { AdminDealRow } from '@/src/server/admin/deals';
import { formatMoney, relativeTimeFrom } from '@/src/lib/format';
import { featureDeals, forceExpireDeals, pauseDeals, reevaluateDeals, resumeDeals } from './actions';
...
type Metrics = {
  ...
  clicks: number;
  ctr: number;
  bySite: Array<{ site: string; impressions: number; clicks: number; ctr: number }>;
  byPartner: Array<{ partner: string; clicks: number }>;
  aiNotes: { available: boolean; note: string };
};
...
function Countdown(props: { expiresAt: string }) {
  ...
  if (hrs >= 1) return <span className="text-xs tabular-nums">{hrs}h {mins % 60}m</span>;
...
export function DealsTable(props: { deals: AdminDealRow[] }) {
  ...
  async function openMetrics(dealId: string) {
    ...
    try {
      const res = await fetch(`/admin/deals/${encodeURIComponent(dealId)}/metrics`, { cache: 'no-store' });
  ...
  function runBulk(kind: 'feature' | 'pause' | 'resume' | 'force_expire' | 'reevaluate') {
    ...
    startTransition(async () => {
      if (kind === 'feature') await featureDeals(selectedIds);
      if (kind === 'pause') await pauseDeals(selectedIds);
      if (kind === 'resume') await resumeDeals(selectedIds);
      if (kind === 'force_expire') await forceExpireDeals(selectedIds);
      if (kind === 'reevaluate') await reevaluateDeals(selectedIds);
      setSelected({});
    });
  }
    ...
    <div className="space-y-3">
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {props.deals.map((d) => {
              ...
              const exp = d.expiresAt instanceof Date ? d.expiresAt : new Date(d.expiresAt);
              ...
              return (
                <tr key={d.id} className="border-t border-slate-200 align-top">
                  ...
                  <td className="px-3 py-2">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-[260px]">
                        ...
                        <div className="mt-0.5 text-xs text-slate-500">
                          {d.product.categoryOverride ?? d.product.category ?? '—'}
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-3 py-2">
                    <Badge tone={d.derived.priority === 'featured' ? 'blue' : d.derived.priority === 'suppressed' ? 'slate' : 'slate'}>
                      {d.derived.priority}
                    </Badge>
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button
                          ...
                          onClick={() => {
                            ...
                            startTransition(() => featureDeals([d.id]));
                          }}
                        >
                          Feature
                        </button>
                        <button
                          ...
                          onClick={() => {
                            ...
                            startTransition(() => (d.suppressed ? resumeDeals([d.id]) : pauseDeals([d.id])));
                          }}
                        >
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            startTransition(() => forceExpireDeals([d.id]));
                          }}
                        >
                          Force expire
                        </button>
                        <button
                          ...
                          onClick={() => {
                            ...
                            startTransition(() => reevaluateDeals([d.id]));
                          }}
                        >
                          Re-evaluate
                        </button>
      ...
      {openId && (
        <div className="fixed inset-0 z-50 flex items-stretch justify-end bg-black/30 p-4" onClick={() => setOpenId(null)}>
          <div
            className="h-full w-full max-w-xl overflow-auto rounded-lg bg-white p-4"
            onClick={(e) => {
              e.stopPropagation();
            }}
          >
