...
async function handle(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  if (path === '/api/posts') {
    ...
    return json(res, 200, {
      ...
      posts: rows.map((p: any) => ({
        ...
        thumbnailSource: p.thumbnailSource ?? null,
        heroImageUrl: p.heroImageUrl ?? null,
        heroImageGeneratedAt: p.heroImageGeneratedAt ? p.heroImageGeneratedAt.toISOString() : null,
        heroImageSource: p.heroImageSource ?? 'placeholder',
  ...
  if (postBySlugMatch) {
    ...
    return json(res, 200, {
      ...
      thumbnailSource: row.thumbnailSource ?? null,
      heroImageUrl: row.heroImageUrl ?? null,
      heroImageGeneratedAt: row.heroImageGeneratedAt ? row.heroImageGeneratedAt.toISOString() : null,
      heroImageSource: row.heroImageSource ?? 'placeholder',
